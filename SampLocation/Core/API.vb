...
Namespace SAMP_API
    Class API
        ...
        Public Const PATH As String = "Open-SAMP-API.dll"
        Public Shared APIExtractPath As String = IO.Path.Combine(IO.Path.GetTempPath, PATH)

        <DllImport("kernel32.dll", SetLastError:=True, CharSet:=CharSet.Ansi)>
        Public Shared Function LoadLibrary(ByVal lpFileName As String) As IntPtr
        End Function

        Public Shared Function ExtractAPIFromResources() As Boolean
            Try

                If IO.File.Exists(APIExtractPath) = False Then

                    IO.File.WriteAllBytes(APIExtractPath, My.Resources.Open_SAMP_API)

                End If

                Dim ModuleHandle As IntPtr = LoadLibrary(APIExtractPath)
                If ModuleHandle = IntPtr.Zero Then
                    If IO.File.Exists(PATH) = False Then
                        IO.File.WriteAllBytes(PATH, My.Resources.Open_SAMP_API)
                        Return True
                    End If
                Else
                    Return True
